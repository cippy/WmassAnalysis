...
void makeCorrelationStudy8TeV(const string& outputDIR_tmp = "./", 
  ...
  if (isMuon) {
    ...
    hvarName.push_back("h2_pfmet_tkmet");
    ...
    xAxisname.push_back("muon p_{T} [GeV]");
    xAxisname.push_back("muon p_{T} [GeV]");
    xAxisname.push_back("PF E_{T}^{miss}");
    ...
    yAxisname.push_back("PF E_{T}^{miss} [GeV]");
    yAxisname.push_back("boson p_{T} [GeV]");
    yAxisname.push_back("tracker E_{T}^{miss}");
